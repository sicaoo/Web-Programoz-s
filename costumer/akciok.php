    ...
    <div id="site_content">
        ...
        <div id="content">
            <h1 style="text-align: center;padding-bottom: 50px; text-decoration: underline;font-weight: bold;">
                Jelenlegi akcióink július 31.-től szeptember 30.-ig tartanak </h1>
            <?php  include ('C:\xampp\htdocs\eden2\db_config.php');


            echo"<div id=\"akciok\">";
            $sql = "SELECT product_name,product_type, description, stock,price, image 
                FROM product
                WHERE cheaper=1;";
            ...
            if ($result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    $type='';
                    if($row['product_type']=="Virágmag")
                    {
                        $type="viragmagok";
                    }
                    elseif ($row['product_type']=="Vetőmag")
                    {
                        $type="vetomagok";
                    }
                    elseif ($row['product_type']=="Gyümölcsfa")
                    {
                        $type="fak";
                    }
                    elseif ($row['product_type']=="Tuja")
                    {
                        $type="fak";
                    }
                    elseif ($row['product_type']=="Fenyő")
                    {
                        $type="fak";
                    }
                    elseif ($row['product_type']=="Kaktusz")
                    {
                        $type="kaktuszok";
                    }
                    else{
                    ...
                    $php_dot='.php';
                    echo"<button type=text><a href='products/".$type."/".$name.$php_dot."'>Tovább</a></button>";
                    echo"</div>";
                }
            } else {
                echo "0 results";
